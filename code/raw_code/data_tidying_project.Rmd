...
The first part of the project consist in prepare the R environment with all the packages that we will be using to acomplish our objectives. Thats what we made with the first code Chunk.
...
After setting up the R environment, we will extract the data using de **readxl** package
...
Saving the data in our repository
...
Wrangling data to clean the information and transform datasets from **messy** to **tidy** (long)
...
Merging the datasets created

```{r merge-data, warning=FALSE} 
## Now, combine the two data frames
df_total <- df_department %>% left_join(df_retail, by = c('business' = 'business')) %>% 
  select(business, year = year.x, n = n.x)
```

##Ploting

Making some plots with the datasets created

```{r plot, warning=FALSE, fig.width=10} 
## Plot Retail Sales data
plot1 <- ggplot(df_retail, aes(x=year,y=n,colour=business)) +
  geom_point() 

## Plot Department Sales data
plot2 <- ggplot(df_department, aes(x=year,y=n)) +
  geom_point() 

## Plot Combined Data
plot3 <- ggplot(df_total, aes(x=year,y=as.numeric(n), colour=business)) +
  geom_point()
```